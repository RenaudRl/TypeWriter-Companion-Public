...
fun onCompanionInteract(event: AsyncFakeEntityInteract, query: Query<CompanionInteractActionEntry>) {
    ...
    // Check if the clicked entity belongs to ANY of the player's companions
    val hasMatchingCompanion = CompanionManager.getAllCompanions(player)
        .any { it.entity.entityId == event.entityId }
    if (!hasMatchingCompanion) return
