package btcrenaud.compagnion.entries.entity
...
class CompanionInvocationDisplay(
    ...
    override fun onPlayerFilterAdded(player: Player) {
        ...
        CompanionManager.summon(
            ...
            activityCreator = act ?: IdleActivity,
            minDistance = minDistance.get(player, context()),
            maxDistance = maxDistance.get(player, context()),
        )
    }

    override fun onPlayerFilterRemoved(player: Player) {
        runCatching { CompanionManager.remove(player, definition.id) }
    }
    ...
    fun onPlayerTeleport(event: PlayerTeleportEvent) {
        ...
        CompanionManager.teleportToPlayer(player, definition.id)
    }
    ...
    fun onPlayerChangedWorld(event: PlayerChangedWorldEvent) {
        ...
        val def = definition.get() ?: return
        val act = activity.get()
        CompanionManager.remove(player, definition.id)
        CompanionManager.summon(
            player = player,
            definition = def,
            definitionRef = definition.id,
    ...
    fun onPlayerQuit(event: PlayerQuitEvent) {
        CompanionManager.removeAll(event.player)
    }
}
